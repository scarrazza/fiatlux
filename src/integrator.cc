...
namespace fiatlux
{
  ...
  //_________________________________________________________________________
  double Integrator::integrate(const double &xmin, const double &xmax, const double &eps, vector<double> const& e) const
  {    
    return dgauss(xmin, xmax, eps, e);
  }

  //_________________________________________________________________________
  double Integrator::dgauss(const double &a, const double &b, const double &eps, vector<double> const& e) const
  {
    int i;
    ...
    for (i = 1; i <= 4; i++)
      {
        u = _x[i-1]*c2;
        s8 += _w[i-1]*(integrand(c1+u, e)+integrand(c1-u, e));
      }

    for (i = 5; i <= 12 ; i++)
      {
        u = _x[i-1]*c2;
        s16 += _w[i-1]*(integrand(c1+u, e)+integrand(c1-u, e));
      }
